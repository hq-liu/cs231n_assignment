...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None, reg=0.0):
    ...
    # shape (N, C).                                                             #
    #############################################################################
    l1 = np.dot(X, W1) + b1  # 第一层输出
    relu = np.maximum(0, l1)  # relu层输出
    l2 = np.dot(relu, W2) + b2  # 第二层输出
    scores = l2
    ...
    #############################################################################
    dscore = np.exp(f) / np.sum(np.exp(f), axis=1, keepdims=True)  # score 的梯度， 用softmax
    dscore[range(N), y] -= 1
    dscore /= N
