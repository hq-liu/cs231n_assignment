...
def rnn_forward(x, h0, Wx, Wh, b):
    ...
    prev_h = h0
    for t in range(T):
        next_h, step_cache = rnn_step_forward(x=x[:, t, :], prev_h=prev_h, Wx=Wx, Wh=Wh, b=b)
        h.append(next_h)
...
def rnn_backward(dh, cache):
    ...
    pass
    (N, T, H) = dh.shape
    D = cache[0][1].shape[0]  # Wx 的shape是(D, H)
    dx = []  # shape(N, T, D)
    ...
    for t in range(T-1, -1, -1):
        stepdx, dprev_h, stepdWx, stepdWh, stepdb = rnn_step_backward(dh[:, t, :] + dprev_h, cache[t])
        dx.append(stepdx)
...
def lstm_step_backward(dnext_h, dnext_c, cache):
    ...
    di = g * dnext_c
    dg = i * dnext_c
    da = np.hstack([i * (1 - i) * di, f * (1 - f) * df, o * (1 - o) * do, (1 - g * g) * dg])  # (N, 4H)

    # a = np.dot(x, Wx) + np.dot(prev_h, Wh) + b  # (N, 4H)
    dx = np.dot(da, Wx.T)  # dx (N, D)
    dWx = np.dot(x.T, da)  # dWx (D, 4H)
...
def lstm_forward(x, h0, Wx, Wh, b):
    ...
    prev_h = h0
    for t in range(T):
        next_h,next_c, step_cache = lstm_step_forward(x=x[:, t, :], prev_h=prev_h, prev_c=prev_c, Wx=Wx, Wh=Wh, b=b)
        h.append(next_h)
        cache.append(step_cache)
        prev_h = next_h
        prev_c = next_c
...
def lstm_backward(dh, cache):
    ...
    db = np.zeros(4 * H)
    for t in range(T-1, -1, -1):
        stepdx, dprev_h, dprev_c, stepdWx, stepdWh, stepdb = lstm_step_backward(dh[:, t, :] + dprev_h, dprev_c, cache[t])
        dx.append(stepdx)
        dWx, dWh, db = dWx+stepdWx, dWh+stepdWh, db+stepdb
    dh0 = dprev_h
    dx = np.array(dx[::-1]).transpose(1, 0, 2)
