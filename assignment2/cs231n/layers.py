...
def batchnorm_forward(x, gamma, beta, bn_param):
    ...
    if mode == 'train':
        ...
        cache = (x, gamma, beta, x_hat, mu,sigma, eps)
...
def batchnorm_backward(dout, cache):
    ...
    x_mu = x - sample_mean
    d_sample_var = np.sum(dx_hat * x_mu, axis=0, keepdims=True) * \
                   (-0.5) * (sample_std_inv ** 3)
    d_sample_mean = -1.0 * np.sum(dx_hat * sample_std_inv, axis=0, keepdims=True) + \
                    d_sample_var * np.mean(x_mu, axis=0, keepdims=True)
    N = x.shape[0]
    dx1 = dx_hat * sample_std_inv
    dx2 = 2.0 * d_sample_var * (x - sample_mean) / N
    dx = dx1 + dx2 + d_sample_mean / N
...
def dropout_forward(x, dropout_param):
    ...
    if mode == 'train':
        ...
        #######################################################################
        mask = (np.random.randn(*x.shape) < (1 - p)) / (1-p)
        out = mask * x
        #######################################################################
        #                           END OF YOUR CODE                          #
        #######################################################################
...
def conv_forward_naive(x, w, b, conv_param):
    ...
    x_pad = np.pad(x, ((0, ), (0, ), (pad, ), (pad, )), 'constant', constant_values=0)  # 对输入数据进行pad
    N, C, H, W = x.shape
    F, C, HH, WW = w.shape
    H_prime = int(1 + (H + 2 * pad - HH) / stride)
    W_prime = int(1 + (W + 2 * pad - WW) / stride)
    out = np.zeros((N, F, H_prime, W_prime))  # 确定输出的尺寸
    for i in range(H_prime):
        for j in range(W_prime):
            x_pad_mask = x_pad[:, :, i * stride: i * stride + HH, j * stride: j * stride + WW]
            # 在x_pad上每隔stride取出一个矩阵进行计算
            for k in range(F):
